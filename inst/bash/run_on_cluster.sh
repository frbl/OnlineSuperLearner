...
if [ $# -eq 0 ]; then
  ...
  for i in $(seq 1 4);
  do
    ssh $server_name -t "cd osl; sbatch -o ~/tmp/$i-output.out -e ~/tmp/$i-error.out --job-name=cfg-$i --ntasks=1 --cpus-per-task=23 --time=03:00:00 --partition=regular --mail-user=peregrine@compsy.nl --mail-type=all --wrap=\"Rscript run.R $i\""
  done
fi
