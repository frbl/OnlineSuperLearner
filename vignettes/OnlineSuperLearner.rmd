...
``` R
  W <- OnlineSuperLearner::RelevantVariable$new(formula = W ~ Y_lag_1 + A_lag_1 +  W_lag_1 + Y_lag_2, family = 'gaussian')
  A <- OnlineSuperLearner::RelevantVariable$new(formula = A ~ W + Y_lag_1 + A_lag_1 + W_lag_1, family = 'binomial')
  Y <- OnlineSuperLearner::RelevantVariable$new(formula = Y ~ A + W, family = 'gaussian')
...
``` R
  ...
  interventionEffectCalculator$evaluate_single_intervention(
    osl = osl,
    initial_data = Data.Static$new(dataset = data.test),
    intervention = intervention,
    tau = tau,
    discrete = discrete
  )
